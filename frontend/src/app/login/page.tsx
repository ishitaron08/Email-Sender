"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import Button from "@/components/ui/Button";

export default function LoginPage() {
  const router = useRouter();
  const [devLoading, setDevLoading] = useState(false);

  function handleGoogleLogin() {
    window.location.href = "/auth/google";
  }

  async function handleDevLogin() {
    setDevLoading(true);
    try {
      const res = await fetch("/auth/dev-login", { method: "POST" });
      if (!res.ok) throw new Error("Dev login failed");
      const { token } = await res.json();
      localStorage.setItem("dispatch_token", token);
      router.push("/compose");
    } catch {
      alert("Dev login failed — make sure backend is running and DB is seeded.");
    } finally {
      setDevLoading(false);
    }
  }

  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="w-full max-w-sm p-8 bg-white rounded-2xl shadow-lg text-center">
        <h1 className="text-2xl font-bold text-gray-800 mb-2">
          Dispatch Engine
        </h1>
        <p className="text-sm text-gray-500 mb-8">
          Schedule & track email campaigns effortlessly.
        </p>

        <Button onClick={handleGoogleLogin} size="lg" className="w-full">
          <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24">
            <path
              fill="currentColor"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
            />
            <path
              fill="currentColor"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="currentColor"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="currentColor"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
          Sign in with Google
        </Button>

        <div className="relative my-6">
          <div className="absolute inset-0 flex items-center">
            <div className="w-full border-t border-gray-200" />
          </div>
          <div className="relative flex justify-center text-xs">
            <span className="bg-white px-2 text-gray-400">or for development</span>
          </div>
        </div>

        <Button
          variant="secondary"
          onClick={handleDevLogin}
          loading={devLoading}
          size="md"
          className="w-full"
        >
          Dev Login (no OAuth)
        </Button>
      </div>
    </div>
  );
}
